...
crc_def.score.winsize    = 20; % in sec
...
% Parameters for Quality Control detection
%--------------------------------------------
% ------------------- preprocessing (pp) ------------------------
% crc_def.qc.pp.
% ------------------- bad channel (bd) -------------------------
% 1/ bad channels (bc) in  EEG
crc_def.qc.bc.tf           = 0.8;   % duration of flat channel (s)
crc_def.qc.bc.n            = 6e3;   % noisy channel threshold (µV)
crc_def.qc.bc.f1           = 1;     % 1st flat channel threshold (µV)
crc_def.qc.bc.ampl         = 0.1;   % threshold for amplitude of flat channel depending on standard deviation
crc_def.qc.bc.r            = 5;     % ratio of deviation
crc_def.qc.bc.f2           = 10;    % 2nd flat channel threshold (µV)
% 2/ bad channels (bc) in  EMG
crc_def.qc.bc.fm           = 1;     % flat channel threshold
crc_def.qc.bc.sm           = 1.5;   % ratio of deviation

% ------------------ bad epochs (be) -------------------------
% -- rapid transition (popping)
crc_def.qc.be.pr    = 5e4;    % variation rate in MAS channel (µV²/sec)
crc_def.qc.be.pe    = 1e5;  % variation rate in EEG channel (µV²/sec)
crc_def.qc.be.pt    = 0.05;   % peak to peak interval (sec)
% -- movement and arousal
crc_def.qc.be.mvt_t = 0.15;     % minimal time to consider movement in nREM(sec)
crc_def.qc.be.ainf  = 3;        % minimal time to consider arousal in nREM (sec)
crc_def.qc.be.asup  = 14;       % macimal time to consider arousal in nREM (sec)
return
