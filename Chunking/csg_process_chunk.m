function fn_chunk = csg_process_chunk(Dmeg,Begpts,Endpts, flags)
...
% Modified by D. Coppieters, 2017 to be adapted to the CSG Toolbox.
...
% Deal with score and other CRC specific bits!
%---------------------------------------------
if isfield(Dmeg,'CRC') && isfield(Dmeg.CRC,'score')
    try
        for nsc = 1:size(Dmeg.CRC.score,2)
            ...
            tmp = Dmeg.CRC.score{5,nsc}(:,1:2)-Begpts/fsample(Dmeg);
            ...
            Dcopy.CRC.score{5,nsc} = [tmp Dmeg.CRC.score{5,nsc}(idxkept,3)] ;
        ...
        fprintf('\nWARNING, could not properly update CRC.score field!!!\n')
        fprintf('Data file is ''%s''',Dcopy.fname)
        fprintf('See error message here under: \n\n')
        rethrow(err);
    end
end
...
% Handling events from the object
%--------------------------------
...
% Copying the data in the new file
%---------------------------------
