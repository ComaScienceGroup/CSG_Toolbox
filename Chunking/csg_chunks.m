function varargout = csg_chunks(varargin)
% CRC_CHUNKS M-file for csg_chunks.fig
...
%      applied to the GUI before csg_chunks_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to csg_chunks_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help csg_chunks
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @csg_chunks_OpeningFcn, ...
    'gui_OutputFcn',  @csg_chunks_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before csg_chunks is made visible.
function csg_chunks_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to csg_chunks (see VARARGIN)

% Choose default command line output for csg_chunks
...
% UIWAIT makes csg_chunks wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = csg_chunks_OutputFcn(hObject, eventdata, handles)
...
csg_process_chunk(handles.Dmeg, Begpts, Endpts, flags);
